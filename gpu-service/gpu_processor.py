...
def signal_handler(sig, frame):
    ...
    logger.info("[GPU] Recebido sinal de encerramento...")
    stop_requested = True
...
def main():
    ...
    def callback(ch, method, properties, body):
        try:
            ...
            if frame is None:
                logger.warning("[GPU] Frame inválido ou corrompido.")
            ...
            if len(cars) > 0:
                logger.info(f"[GPU] Carros detectados: {len(cars)}. Detalhes: {cars}")
            else:
                logger.info("[GPU] Nenhum carro detectado neste frame.")

            ch.basic_ack(delivery_tag=method.delivery_tag)

        except Exception as e:
            logger.exception("[GPU] Erro no processamento:")
            ch.basic_ack(delivery_tag=method.delivery_tag)

    channel.basic_consume(
        queue=GPU_TASKS_QUEUE,
        on_message_callback=callback,
        auto_ack=False
    )

    logger.info("[GPU] Aguardando tarefas de GPU...")
    try:
        channel.start_consuming()
    except KeyboardInterrupt:
        logger.info("[GPU] Interrompido via teclado.")

    monitor.stop()
    monitor.join()
    connection.close()
    logger.info("[GPU] Encerrado com sucesso.")
